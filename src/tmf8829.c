...
#define CLK_IDX_MOD(i) ((uint8_t)((i) & (uint8_t)((TMF8829_CLK_CORRECTION_PAIRS) - 1u)))
 ...
 */
static int reg_poll_first(tmf8829_driver_t* drv, uint8_t reg, uint8_t expected, uint8_t len, uint16_t timeout_ms) {
...
static uint16_t clk_calc_ratio_uq(uint32_t h_diff, uint32_t t_diff, uint16_t prev, uint32_t host_per_ms) {
  ...
  if (h_diff >= ht && t_diff >= (uint32_t)TMF8829_TICKS_PER_1000_US) {
    ...
    if (t_diff != 0u) {
      ...
      if (ratio_uq32 > (uint32_t)(1u << 15) + (1u << 14) || ratio_uq32 < (uint32_t)(1u << 15) - (1u << 14)) {
        ratio_uq32 = prev;
      }
    }
  }
...
static void clk_add_pair(tmf8829_driver_t* drv, uint32_t host_tick, uint32_t device_tick) {
  uint8_t idx;
  uint8_t idx_old;
  uint32_t h_diff;
  uint32_t t_diff;
  ...
  drv->_clk_corr_ratio_uq = clk_calc_ratio_uq(h_diff, t_diff, drv->_clk_corr_ratio_uq, hpm);
}
...
int tmf8829_wakeup(tmf8829_driver_t* drv) {
  ...
  if ((en & TMF8829_ENABLE_CPU_READY_MASK) == 0u) {
    en = (uint8_t)(en | (uint8_t)(TMF8829_POWERUP_RAM << TMF8829_ENABLE_POWERUP_SHIFT));
...
int tmf8829_is_device_wakeup(tmf8829_driver_t* drv, uint8_t* out_awake) {
  uint8_t en;
  if (!tmf8829_internal_is_initialised(drv) || out_awake == NULL) {
    return TMF8829_E_PARAM;
  }
  if (bus_read(drv, TMF8829_REG_ENABLE, &en, 1) != TMF8829_OK) {
    return TMF8829_E_BUS;
  }
  *out_awake = (en & TMF8829_ENABLE_CPU_READY_MASK) ? 1u : 0u;
  return TMF8829_OK;
}
...
int tmf8829_read_device_info(tmf8829_driver_t* drv) {
  ...
  if (bus_read(drv, TMF8829_REG_SERIAL_NUMBER_0, drv->buffer, 4) != TMF8829_OK) {
    return TMF8829_E_BUS;
  }
...
int tmf8829_command(tmf8829_driver_t* drv, uint8_t cmd, uint16_t timeout_ms) {
  ...
  rc = reg_poll_first(drv, TMF8829_REG_CMD_STAT, TMF8829_STAT_OK, 1u, timeout_ms);
  return rc;
}
...
int tmf8829_cmd_load_config_page(tmf8829_driver_t* drv) {
  ...
  rc = reg_poll_first(drv, TMF8829_REG_CMD_STAT, TMF8829_STAT_OK, 1u, TMF8829_APP_CMD_LOAD_CONFIG_TIMEOUT_MS);
  if (rc != TMF8829_OK) {
    return rc;
  }
  return reg_poll_first(drv, TMF8829_REG_CID_RID, TMF8829_CMD_LOAD_CONFIG_PAGE, 1u, TMF8829_APP_CMD_LOAD_CONFIG_TIMEOUT_MS);
}
...
int tmf8829_cmd_write_page(tmf8829_driver_t* drv) {
  ...
  return reg_poll_first(drv, TMF8829_REG_CMD_STAT, TMF8829_STAT_OK, 1u, TMF8829_APP_CMD_WRITE_CONFIG_TIMEOUT_MS);
}
...
int tmf8829_get_configuration(tmf8829_driver_t* drv) {
  ...
  return bus_read(drv, TMF8829_REG_CFG_PERIOD_MS_LSB, drv->config, (uint16_t)sizeof(drv->config));
}
...
int tmf8829_set_configuration(tmf8829_driver_t* drv) {
  ...
  rc = bus_write(drv, TMF8829_REG_CFG_PERIOD_MS_LSB, drv->config, (uint16_t)sizeof(drv->config));
  if (rc != TMF8829_OK) {
    return rc;
  }
...
int tmf8829_start_measurement(tmf8829_driver_t* drv) {
  ...
  rc = reg_poll_first(drv, TMF8829_REG_CMD_STAT, TMF8829_STAT_ACCEPTED, 1u, TMF8829_APP_CMD_MEASURE_TIMEOUT_MS);
  if (rc != TMF8829_OK) {
    return rc;
  }

  period = (uint16_t)drv->config[0] + (uint16_t)((uint16_t)drv->config[TMF8829_REG_CFG_PERIOD_MS_MSB - TMF8829_REG_CFG_PERIOD_MS_LSB] << 8);
  if (period != 0u) {
...
int tmf8829_stop_measurement(tmf8829_driver_t* drv) {
  ...
  drv->_cyclic_running = 0u;
  return reg_poll_first(drv, TMF8829_REG_CMD_STAT, TMF8829_STAT_OK, 1u, TMF8829_APP_CMD_STOP_TIMEOUT_MS);
}
...
int tmf8829_bootloader_spi_off(tmf8829_driver_t* drv) {
  ...
  return reg_poll_first(drv, TMF8829_REG_CMD_STAT, TMF8829_BL_STAT_READY, 1u, TMF8829_BL_CMD_TIMEOUT_MS);
}
...
static int bl_set_ram_addr(tmf8829_driver_t* drv, uint32_t addr) {
  ...
  return reg_poll_first(drv, TMF8829_REG_CMD_STAT, TMF8829_BL_STAT_READY, 3u, TMF8829_BL_SET_ADDR_TIMEOUT_MS);
}

static int bl_write_ram_both(tmf8829_driver_t* drv, uint8_t payload_len, const uint8_t* payload) {
  ...
  return reg_poll_first(drv, TMF8829_REG_CMD_STAT, TMF8829_BL_STAT_READY, 1u, TMF8829_BL_W_RAM_TIMEOUT_MS);
}
...
static int bl_write_fifo_both(tmf8829_driver_t* drv, uint32_t addr, uint32_t len) {
  ...
  return reg_poll_first(drv, TMF8829_REG_CMD_STAT, TMF8829_BL_STAT_READY, 1u, TMF8829_BL_W_FIFO_TIMEOUT_MS);
}
...
static int bl_start_ram_app(tmf8829_driver_t* drv) {
  ...
  return reg_poll_first(drv, TMF8829_REG_APP_ID, TMF8829_APP_ID__APPLICATION, 1u, TMF8829_BL_START_APP_TIMEOUT_MS);
}
...
int tmf8829_download_firmware(tmf8829_driver_t* drv, uint32_t image_start_addr, int use_fifo) {
  ...
  n = drv->fw_image_read(drv, 0u, NULL, 0u, &image_size);
  if (n < 0) {
    return n;
  }
  if (image_size == 0u) {
  ...
  if (use_fifo) {
    ...
    while (idx < image_size) {
      room = TMF8829_BL_MAX_PAYLOAD;
      maxw = drv_max_write_len(drv);
      ...
      if (bus_write(drv, TMF8829_REG_FIFO, drv->buffer, chunk_len) != TMF8829_OK) {
        return TMF8829_E_BUS;
      }
      idx += chunk_len;
    }
    ...
    while (idx < image_size) {
      ...
      uint32_t max_payload_by_bus;

      maxw = drv_max_write_len(drv);
      if (maxw <= TMF8829_BL_WR_HEADER) {
        return TMF8829_E_PARAM;
      }
      max_payload_by_bus = (uint32_t)maxw - TMF8829_BL_WR_HEADER;
      ...
      n = drv->fw_image_read(drv, idx, &drv->buffer[TMF8829_BL_WR_HEADER], chunk_len, NULL);
      if (n < 0) {
        return n;
      }
      if ((uint32_t)n != (uint32_t)chunk_len) {
      ...
      rc = bl_write_ram_both(drv, (uint8_t)chunk_len, &drv->buffer[TMF8829_BL_WR_HEADER]);
      if (rc != TMF8829_OK) {
        return rc;
      }
      idx += chunk_len;
...
int tmf8829_read_results(tmf8829_driver_t* drv) {
  ...
  const uint16_t hdr_read = (uint16_t)(TMF8829_PRE_HEADER_SIZE + TMF8829_FRAME_HEADER_SIZE);
  ...
  if (bus_read(drv, TMF8829_REG_FIFO_STATUS, drv->buffer, hdr_read) != TMF8829_OK) {
    return TMF8829_E_BUS;
  }
  ...
  if ((drv->buffer[TMF8829_PRE_HEADER_SIZE] & TMF8829_FID_MASK) != TMF8829_FID_RESULTS) {
    return TMF8829_E_NO_RESULT;
  }

  payload      = tmf8829_get_uint16(drv->buffer + 7u);
  size_to_read = (uint16_t)(payload - (TMF8829_FRAME_HEADER_SIZE - TMF8829_FRAME_HEADER_OFFSET));

  rx_ok = TMF8829_OK;
  do {
    if (pixel_size > 0u && size_to_read > drv->buffer_len) {
      rx_size = (uint16_t)(((uint16_t)(drv->buffer_len / pixel_size)) * pixel_size);
    ...
    if (bus_read(drv, TMF8829_REG_FIFO, drv->buffer, rx_size) != TMF8829_OK) {
      rx_ok = TMF8829_E_BUS;
      break;
    }
    ...
    if (drv->_clk_corr_enable) {
      ...
      if (size_to_read < TMF8829_FRAME_FOOTER_SIZE) {
        uint16_t trim = (uint16_t)(TMF8829_FRAME_FOOTER_SIZE - size_to_read);
        if (rx_size > trim) {
  ...
  /*
   * EOF validation: the last two bytes of the final chunk must contain the
   * frame end marker. This assumes the device emits the footer within a single
   * read chunk (vendor driver makes the same assumption). If the footer were
   * split across two reads, the marker would not appear contiguously here.
   */
  if (rx_size < TMF8829_FRAME_EOF_SIZE) {
    return TMF8829_E_NO_RESULT;
  }
  eof_marker = tmf8829_get_uint16(drv->buffer + rx_size - TMF8829_FRAME_EOF_SIZE);
...
int tmf8829_read_histogram(tmf8829_driver_t* drv) {
  ...
  if ((drv->buffer[TMF8829_PRE_HEADER_SIZE] & TMF8829_FID_MASK) != TMF8829_FID_HISTOGRAMS) {
    return TMF8829_E_NO_RESULT;
  }
...
uint8_t tmf8829_get_pixel_size(uint8_t layout) {
  ...
  uint8_t use_signal = (layout & TMF8829_RESULT_FORMAT_SIG_STRENGTH_MASK) ? 1u : 0u;
  ...
  size = (uint8_t)((num_peak * (uint8_t)(3u + 2u * use_signal)) + (uint8_t)(2u * use_noise) + (uint8_t)(2u * use_xtalk));
  return size;
}
...
void tmf8829_correct_distance_data_segment(tmf8829_driver_t* drv, uint16_t size, uint8_t layout) {
  ...
  if (size > drv->buffer_len) {
    size = drv->buffer_len;
  }
  num_peaks = (uint8_t)(layout & TMF8829_RESULT_FORMAT_NR_PEAKS_MASK);

  for (idxpixel = 0u; (uint16_t)(idxpixel + pixel_size) <= size; idxpixel = (uint16_t)(idxpixel + pixel_size)) {
    uint8_t idx = 0u;
    uint8_t idx_peak;
...
int tmf8829_set_i2c_address(tmf8829_driver_t* drv, uint8_t new_addr) {
  ...
  if (drv->bus != TMF8829_BUS_I2C) {
    return TMF8829_E_PARAM;
  }
  if (new_addr > 0x7Fu) {
    return TMF8829_E_PARAM;
  }
  if (bus_write(drv, TMF8829_REG_I2C_DEVADDR, &new_addr, 1) != TMF8829_OK) {
    return TMF8829_E_BUS;
  }
  drv->i2c_addr = new_addr;
  return TMF8829_OK;
}
