...
TEST_CASE("get_and_clr_interrupts: nothing pending issues only a read",
          "[tmf8829][irq]")
{
    Fixture f;
    ...
    REQUIRE(tmf8829_get_and_clr_interrupts(&f.drv, TMF8829_INT_ALL, &pending)
            == TMF8829_OK);
    REQUIRE(pending == 0u);
...
TEST_CASE("get_and_clr_interrupts: pending bits are returned and acked",
          "[tmf8829][irq]")
{
    Fixture f;
    REQUIRE(f.init() == TMF8829_OK);
    ...
    const std::uint8_t raised =
        TMF8829_INT_RESULTS | TMF8829_INT_HISTOGRAMS;
...
TEST_CASE("get_and_clr_interrupts: bits outside the mask are not cleared",
          "[tmf8829][irq]")
{
    Fixture f;
    REQUIRE(f.init() == TMF8829_OK);
    ...
    f.bus.preload(TMF8829_REG_INT_STATUS,
                  TMF8829_INT_RESULTS | TMF8829_INT_MOTION);

    std::uint8_t pending = 0xFF;
    // Caller is only interested in result frames.
    REQUIRE(tmf8829_get_and_clr_interrupts(&f.drv, TMF8829_INT_RESULTS, &pending)
            == TMF8829_OK);
...
TEST_CASE("get_and_clr_interrupts: read failure yields E_BUS",
          "[tmf8829][irq]")
{
    Fixture f;
    REQUIRE(f.init() == TMF8829_OK);
...
TEST_CASE("get_and_clr_interrupts: write failure yields E_BUS",
          "[tmf8829][irq]")
{
    Fixture f;
    REQUIRE(f.init() == TMF8829_OK);
...
TEST_CASE("get_and_clr_interrupts: rejects null out pointer",
          "[tmf8829][irq]")
{
    Fixture f;
    REQUIRE(f.init() == TMF8829_OK);
    REQUIRE(tmf8829_get_and_clr_interrupts(&f.drv, TMF8829_INT_ALL, nullptr)
            == TMF8829_E_PARAM);
    // Implementation must not touch the bus on parameter rejection.
    REQUIRE(f.bus.ops.empty());
}

TEST_CASE("get_and_clr_interrupts: rejects uninitialised driver",
          "[tmf8829][irq]")
{
    tmf8829_driver_t drv{};
    std::uint8_t pending = 0u;
    REQUIRE(tmf8829_get_and_clr_interrupts(&drv, TMF8829_INT_ALL, &pending)
            == TMF8829_E_PARAM);
}
