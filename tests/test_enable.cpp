...
TEST_CASE("is_cpu_ready returns TIMEOUT after exhausting the budget", "[tmf8829][cpu_ready]") {
  Fixture f;
  REQUIRE(f.init() == TMF8829_OK);
  // Bit never set.
...
TEST_CASE("is_cpu_ready returns BUS error when read fails", "[tmf8829][cpu_ready]") {
  Fixture f;
  REQUIRE(f.init() == TMF8829_OK);
