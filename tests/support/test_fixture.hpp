...
public:
    ...
    explicit Fixture(tmf8829_bus_t bus_kind = TMF8829_BUS_I2C,
                     std::uint8_t  i2c_addr = TMF8829_DEFAULT_I2C_ADDR)
    {
...
private:
    static Fixture *self(tmf8829_driver_t *drv)
    {
        return static_cast<Fixture *>(drv->user_ctx);
    }

    static int c_read(tmf8829_driver_t *drv,
                      std::uint8_t reg, std::uint8_t *buf, std::uint16_t len)
    {
        return self(drv)->bus.do_read(reg, buf, len);
    }
    static int c_write(tmf8829_driver_t *drv,
                       std::uint8_t reg, const std::uint8_t *buf, std::uint16_t len)
    {
        return self(drv)->bus.do_write(reg, buf, len);
    }
    static void c_write_pin_enable(tmf8829_driver_t *drv, int high)
    {
        self(drv)->pin.write_pin_enable(high);
    }
    static int c_read_pin_int(tmf8829_driver_t *drv)
    {
        return self(drv)->pin.read_pin_int();
    }
};
