...
TEST_CASE("default_image_read: size-only probe", "[tmf8829][image]") {
  std::uint32_t sz = 0u;
  REQUIRE(tmf8829_default_image_read(nullptr, 0u, nullptr, 0u, &sz) == 0);
  REQUIRE(sz == TMF8829_VENDOR_IMAGE_NUM_BYTES);
}

TEST_CASE("default_image_read: first loader bytes", "[tmf8829][image]") {
  std::uint8_t chunk[4]{};
  REQUIRE(tmf8829_default_image_read(nullptr, 0u, chunk, sizeof chunk, nullptr) == 4);
...
TEST_CASE("default_image_read: offset past end returns zero", "[tmf8829][image]") {
  std::uint8_t one = 0u;
  REQUIRE(tmf8829_default_image_read(nullptr, TMF8829_VENDOR_IMAGE_NUM_BYTES, &one, 1u, nullptr) == 0);
}
